...
from mongoengine import connect, EmbeddedDocument, Document, disconnect
...
disconnect()
connect(db=config.db_name, host=config.db_host, port=config.db_port)


# # --------------------------
# # MongoDB connection for inside docker
# # --------------------------
# connect(
#     db="ecommerce",
...
class Product(Document):
    ...
    def clean(self):
        ...
        if self.images and not self.primary_image:
            primary_img = next(
                (img for img in self.images if img.is_primary), self.images[0]
            )
...
class User(Document):
    ...
    meta = {
        "collection": "users",
        "indexes": ["email", "username", "created_at", "is_admin", "is_staff"],
    }
    ...
    # Preferences
    email_preferences = DictField(
        default={"promotional": True, "order_updates": True, "price_drops": True}
    )
...
class Order(Document):
    ...
    meta = {
        "collection": "orders",
        "indexes": ["order_number", "user", "status", "created_at", "email"],
    }
...
class Session(Document):
    ...
    meta = {
        "collection": "sessions",
        "indexes": ["session_id", "user_id", "session_type", "expires_at"],
    }
...
class SessionManager:
    @staticmethod
    def create_session(
        user_id: str = None, session_type: str = "user", user_data: dict = None
    ) -> str:
