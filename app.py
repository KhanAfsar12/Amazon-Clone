...
def home(request: Request):
    ...
    for category in category_list:
        ...
        products = Product.objects(category__in=category_ids, is_active=True).order_by(
            "-created_at"
        )[:6]
...
async def admin_login(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "admin/login.html", {"request": request, "error": e.detail}
        )
...
async def admin_model_list(
    ...
    if filter_field and filter_field in config.list_filter and filter_value:

        if filter_field in config.list_filter:
            if filter_value.lower() == "true":
                filter_value_converted = True
            elif filter_value.lower() == "false":
                filter_value_converted = False
            else:
                filter_value_converted = None

            if filter_value_converted is not None:
                query &= Q(**{filter_field: filter_value_converted})
                print(f"Filter applied: {filter_field} = {filter_value_converted}")
        else:
...
async def admin_model_create(request: Request, model_name: str):
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/{model_name}", status_code=status.HTTP_302_FOUND
        )
                ...
                if field and isinstance(field, BooleanField):
                    ...
                    if field_name == "is_admin" and boolean_value == True:
                        (
                            ...
                            session_user["user_data"]["is_admin"],
                        ) = ("admin", True)
                        session_user.save()
